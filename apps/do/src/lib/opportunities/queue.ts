...
/**
 * Mark an opportunity as accepted (the user opened the editor and
 * is writing a reply). The opportunity row stays accepted until the
 * reply is posted or the editor is closed without posting.
 */
export async function acceptOpportunity(
  ...
  return getOpportunityById(supabase, userId, opportunityId);
}
