...
- [ ] `pnpm db:types` — deferred to a **post-merge follow-up**. Concrete reason (verifiable in the repo): the root `package.json` `db:types` script generates from the **linked prod project** (`supabase gen types --project-id ioptgqtzykqwnebwkioo`), so running it before these migrations reach prod would emit types for a schema that lacks the new tables. There is also no local Supabase stack in the authoring environment. The proper sequence is: merge → `supabase db push` (git-sync-gated) applies 00087–00090 to prod → `pnpm db:types` → commit regenerated `packages/supabase/src/types.ts` in the follow-up. Until then, code that reads the new tables (embed persistence, 8.4/8.5) is not written, so the missing types block nothing.
