...
| `SLACK_CLIENT_ID` / `SLACK_CLIENT_SECRET` | yes | no | **Load-bearing since D1.** Slack OAuth v2 client for the `slack` system connection. Create at api.slack.com/apps: add redirect URL `https://kinetiks.ai/api/connections/system/slack/callback`; bot scopes are requested by the app (`chat:write`, `app_mentions:read`, channel/IM read + write, `users:read`). Unset = the Slack card renders "Not configured for this deployment yet." |
| `SLACK_BOT_TOKEN` | transitional | no | Workspace-global bot token still read by `lib/slack/dispatch.ts` (the `send_slack_notification` tool). D2 replaces it with the per-account encrypted bot token from the D1 `slack` connection; until then it remains the dispatcher's token source. |
