...
| `SLACK_SIGNING_SECRET` | yes | no | **Load-bearing since D3.** Verifies every inbound Slack request (`/api/slack/events`, `/api/slack/interactive`) - HMAC v0 scheme + 5-minute replay window. From the Slack app's Basic Information page. In the Slack app config: Event Subscriptions → Request URL `https://kinetiks.ai/api/slack/events` (subscribe to `app_mention`, `message.im`); Interactivity → Request URL `https://kinetiks.ai/api/slack/interactive`. Unset = both routes refuse with 503. |
